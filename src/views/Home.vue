<template>
  <div class="home-page">
    <section class="hero">
      ...
      <h2>Passive monitoring for spaces that matter.</h2>
      <nav class="audience-links" aria-label="Explore Thoth by use case">
        <button v-for="(item, i) in uses" :key="item.title" @click="openCard(item, i)">{{ item.title }} ↗</button>
      </nav>
      <div class="hero-foot">
...
.hero h2{font-size:clamp(66px,8.6vw,132px);line-height:.89;letter-spacing:-.072em;font-weight:620;max-width:1500px;margin:54px 0 40px}.audience-links{display:flex;flex-wrap:wrap;gap:9px;margin-bottom:60px}.audience-links button{border:1px solid #11110f;border-radius:999px;background:transparent;color:#11110f;padding:11px 17px;font:650 14px inherit;cursor:pointer;transition:background .2s,color .2s}.audience-links button:hover{background:#11110f;color:#f4f1e9}@media(max-width:800px){.hero h2{font-size:clamp(53px,16vw,78px);margin:42px 0 32px}.audience-links{margin-bottom:44px}}
</style>
